...
class TextureShaderWindow(WindowConfig):
    ...
    resource_dir = (pathlib.Path(__file__).parent / 'shader').resolve()
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.texture_renderer = self.load_program(
            vertex_shader=f'{self.shader_dirs[0]}/vertex_shader.glsl',
            fragment_shader=f'{self.shader_dirs[0]}/fragment_shader.glsl'
        )

        # compute shader
        self.compute_shader = self.load_compute_shader(
            f'{self.shader_dirs[0]}/compute_shader.glsl',
            defines={
                'destText': 0
            }
        )
        # clr_fg needs to be passed to the compute shader initially, because it is a uniform
        self.compute_shader['clr_fg'] = appConfig.clr_fg_rgb
    ...
    def render_simulation_frame(self, time: float) -> None:
        ...
        self.ctx.clear(*appConfig.clr_bg_rgb)
    ...
    def render_ui_frame(self) -> None:
        ...
        if imgui.begin('COLORS'):
            ...
            changed, appConfig.clr_fg_rgb = imgui.color_edit3(
                "fg", *appConfig.clr_fg_rgb
            )
            imgui.end_child()
            ...
            changed, appConfig.clr_bg_rgb = imgui.color_edit3(
                "bg", *appConfig.clr_bg_rgb
            )
            imgui.end_child()
            if changed:  # pass the new value to the compute shader
